...
export function EditorModal<T>({ show, ...props }: EditorModalProps<T>) {
    ...
    return (
        <EditorModalInner key={key} show={show} {...props} />
    );
}

function EditorModalInner<T>({ onRequestHide, show, setValueAndHide, value: initial, children }: EditorModalProps<T>) {
    ...
    return (
        <Modal onRequestHide={onRequestHide} show={show}>
            ...
            <ModalFooter>
                <ModalButton className={buttonStyles.green} onClick={() => setValueAndHide(state[0])}>
                    OK
                </ModalButton>
                <ModalButton onClick={onRequestHide}>
                    Cancel
                </ModalButton>
            </ModalFooter>
        </Modal>
